...
const products = [
  {
    ...
    name: "fireworks",
    imageUri:
      "https://thumbs.dreamstime.com/z/beautiful-colorful-firework-display-night-celebrate-anniversary-166189071.jpg"
  },
  {
    id: 2,
    name: "random image",
...
class Main extends Component {
  constructor(props) {
    ...
    this.state = { isSelected: false, addToCartCollection: [] };
  }

  _addToCart = (id, name) => {
    this.setState({
      addToCartCollection: [...this.state.addToCartCollection, { id, name }]
    });
  };
  _removeToCart = id => {
    this.setState({
      addToCartCollection: this.state.addToCartCollection.filter(
        citem => citem.id !== id
      )
    });
  };
  ...
  render() {
    const { addToCartCollection } = this.state;

    console.log("main console", this.props);
    return (
      <div
        style={{
          width: "100%",
      ...
      >
        ...
        <AddToCartWrapper>{addToCartCollection.length}</AddToCartWrapper>
        {products.map(item => (
          <Product
            item={item}
            key={item.id}
            ...
            removeToCart={this._removeToCart}
            goToCart={this._gotoCart}
          />
        ))}
      </div>
    );
